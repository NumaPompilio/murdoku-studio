...
(function (root) {
  ...
  function isFrontWindow(m, r, c) {
    return (r > 0 && isWindow(m, r, c, r-1, c)) ||
           (r < m.size - 1 && isWindow(m, r, c, r+1, c)) ||
           (c > 0 && isWindow(m, r, c, r, c-1)) ||
           (c < m.size - 1 && isWindow(m, r, c, r, c+1));
  }
    ...
    for (const e of sus) {
      ...
      if (allow("OBJECT_DIR")) {
        const dirs = [
          ...
          { d: "E", dr: 0, dc: -1, lbl: "esattamente a est" },
          { d: "W", dr: 0, dc: 1, lbl: "esattamente a ovest" }
        ];
            ...
            if (k) {
              out.push(clue("OBJECT_DIR", e, { objectKind: k, dir: dir.d }, "oggetto", 3,
                ...
                ctx => ctx.occCells.filter(o => {
                  ...
                  return inBounds(ctx.map, tr, tc) && cellAt(ctx.map, tr, tc).object === k;
                })));
            }
          }
        }
      ...
      for (const o of sus) {
        ...
        if (allow("CARDINAL")) {
          ...
          const labelEW = e.col < o.col ? "ovest" : "est";
...
const OBJ_CATEGORIES = {
  ...
  beach: {
    ...
    list: ["umbrella", "sunbed", "towel", "mattress", "surfboard", "walkway", "pedalo", "dock", "boat", "cabin", "lifeguard", "palm", "shower", "kiosk", "volleyball", "cooler", "scoglio", "boa"]
  },
  house: {
    label: "🏠 Casa & Interni",
    list: ["chair", "table", "bed", "rug", "bookcase", "shelf", "tv", "plant"]
  },
  nature: {
    label: "🌳 Natura & Esterni",
    list: ["tree", "palm", "flowers", "statue", "boulder", "scoglio", "car"]
  },
  walkable: {
...
function render(){
  ...
  for(const a in centroid){ const g=centroid[a]; const cx=P+(g.sx/g.n+0.5)*CS, cy=P+(g.sy/g.n+0.5)*CS;
    ...
    s += `<text x="${cx}" y="${cy}" font-size="10" fill="#6a5f4a" text-anchor="middle" opacity="0.6" pointer-events="none" font-family="var(--mono)" style="text-transform:uppercase; letter-spacing:1px; font-weight:700">${escapeXml(nm)}</text>`;
  }
...
function renderClues(){
  ...
  for(const s of sus){
    ...
    html+=`<div class="grp"><div class="gh"><span class="badge" style="background:${color}">${s.initial}</span> ${escapeHtml(s.name)} <span style="text-transform:none; font-size:10.5px; opacity:0.8; margin-left:6px">${traitsText}</span></div>`;
...
function buildPrintObjectsLegend(m){
  ...
  let html = `<div class="p-h" style="margin:10px 0 4px">Legenda elementi e oggetti</div><div class="p-obj-grid">`;
  for(const k of used){
    const walk = E.objWalkable(k);
    html += `<div class="p-obj-item">
      ...
      <span class="p-obj-name"><b>${escapeHtml(E.objLabel(k))}</b> <span class="p-obj-type">(${walk ? "calpestabile" : "ostacolo"})</span></span>
    </div>`;
  }
  if(hasBlocked){
    html += `<div class="p-obj-item">
      <span style="display:inline-block;width:20px;height:20px;background:#333;border-radius:4px;border:1px solid #111"></span>
      <span class="p-obj-name"><b>Cella bloccata</b> <span class="p-obj-type">(non calpestabile)</span></span>
    </div>`;
  }
  html += `</div>`;
  return html;
...
$("#doPrint").addEventListener("click",()=>{
  ...
  for(const s of sus){ 
    ...
    clueHtml+=`<div class="p-char-box">
      <div class="p-char-h"><span style="background:#555; color:#fff; padding:1px 5px; border-radius:3px; margin-right:5px">${s.initial}</span> ${escapeHtml(s.name)}</div>
      <div class="p-char-traits">${traitsText}</div>`;
    if (list.length) {
  ...
  if(globals.length){ 
    clueHtml+=`<div class="p-char-box">
      <div class="p-char-h">Vincoli Globali</div>
      <div class="p-char-traits">(Si applicano all'intera mappa)</div>`;
  ...
  $("#printArea").innerHTML=`
    <div class="p-sheet">
      ...
      ${objLegendHtml}
      <div class="p-h">Indizi & Personaggi</div>
...
function boardSVGForPrint(withSolution){
  ...
  if(withSolution){ for(const e of m.entities){ const x=P+e.col*CS,y=P+e.row*CS,cx=x+CS/2,cy=y+CS/2;
    const traits = (e.gender==='M'?'👨':'👩')+(e.hasHat?'🧢':'')+(e.hasGlasses?'👓':'');
    if(e.kind==="victim"){ s+=`<circle cx="${cx}" cy="${cy}" r="${CS*0.33}" fill="none" stroke="#c00" stroke-width="2" stroke-dasharray="3 2"/><text x="${cx}" y="${cy+2}" font-size="16" text-anchor="middle" fill="#c00">✖</text><text x="${cx}" y="${cy+12}" font-size="9" text-anchor="middle">${traits}</text>`; }
    else { s+=`<circle cx="${cx}" cy="${cy}" r="${CS*0.31}" fill="${e.isMurderer?'#c00':'#333'}"/><text x="${cx}" y="${cy+2}" font-size="15" text-anchor="middle" fill="#fff" font-weight="700">${e.initial}</text><text x="${cx}" y="${cy+12}" font-size="9" text-anchor="middle">${traits}</text>`; } } }
